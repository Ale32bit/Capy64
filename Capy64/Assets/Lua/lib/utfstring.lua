...
local function toUTF8(s)
    ...
    while i do
        local oi = i
        ok, i, c = pcall(iter, invar, i)
        if not ok then i, ss = oi + 1, ss .. utf8.char(string.byte(s, utf8.offset(s, oi) + 1))
        elseif c then ss = ss .. utf8.char(c) end
    end
    return ss
end
...
function UTFString:byte(i, j)
    ...
    return utf8.codepoint(self.str, i, j)
end

UTFString.char = utf8.char
UTFString.dump = string.dump

function UTFString:find(pattern, init, plain)
...
function UTFString:format(...)
    ...
    for i = 1, args.n do
        if type(args[i]) == "table" and getmetatable(args[i]) == UTFString_mt then args[i] = args[i].str
        elseif type(args[i]) == "string" then args[i] = toUTF8(args[i]) end
    end
    return UTFString(string.format(self.str, table.unpack(args, 1, args.n)))
end
...
function UTFString:gmatch(pattern)
    ...
    return function()
        ...
        for i = 1, matches.n do
            ...
            elseif tt == "number" then matches[i] = utf8.len(string.sub(self.str, 1, matches[i])) + 1 end
        end
...
function UTFString:len()
    return utf8.len(self.str)
end

function UTFString:lower()
    local s = ""
    for _, c in utf8.codes(self.str) do
        if utf8_case_conv_utl[c] then
            if type(utf8_case_conv_utl[c]) == "table" then s = s .. utf8.char(table.unpack(utf8_case_conv_utl[c]))
            else s = s .. utf8.char(utf8_case_conv_utl[c]) end
        else s = s .. utf8.char(c) end
    end
    return UTFString(s)
end
...
function UTFString:pack(...)
    return UTFString(string.pack(self.str, ...))
end

function UTFString:packsize()
    return string.packsize(self.str)
end
...
function UTFString:rep(n, sep)
    ...
    if type(sep) == "table" then
        if getmetatable(sep) ~= UTFString_mt then error("bad argument #2 (expected string or UTFString, got table)", 2) end
        sep = sep.str
    elseif sep then sep = toUTF8(sep) end
    return UTFString(string.rep(self.str, n, sep))
end
...
function UTFString:sub(i, j)
    expect(1, i, "number")
    expect(2, j, "number", "nil")
    return UTFString(string.sub(self.str, utf8.offset(self.str, i), j and utf8.offset(self.str, j)))
end

function UTFString:unpack(s, pos)
    expect(1, s, "string", "table")
    expect(2, pos, "number", "nil")
    if type(s) == "table" then
        if getmetatable(s) ~= UTFString_mt then error("bad argument #1 (expected string or UTFString, got table)", 2) end
        s = s.str
        if pos then pos = utf8.offset(s, pos) end
    end
    return string.unpack(self.str, s, pos)
end

function UTFString:upper()
    local s = ""
    for _, c in utf8.codes(self.str) do
        if utf8_case_conv_ltu[c] then
            if type(utf8_case_conv_ltu[c]) == "table" then s = s .. utf8.char(table.unpack(utf8_case_conv_ltu[c]))
            else s = s .. utf8.char(utf8_case_conv_ltu[c]) end
        else s = s .. utf8.char(c) end
    end
    return UTFString(s)
end
...
function UTFString_mt:__len()
    return utf8.len(self.str)
end

function UTFString_mt.__concat(a, b)
...
function UTFString_mt.__eq(a, b)
    if type(a) == "string" then return a == b.str
    elseif type(b) == "string" then return a.str == b
    elseif type(a) ~= "table" or type(b) ~= "table" or getmetatable(a) ~= UTFString_mt or getmetatable(b) ~= UTFString_mt then return false
    else return a.str == b.str end
end

function UTFString_mt.__lt(a, b)
    if type(a) == "string" then return a < b.str
    elseif type(b) == "string" then return a.str < b
    else
...
function UTFString_mt.__le(a, b)
    if type(a) == "string" then return a <= b.str
    elseif type(b) == "string" then return a.str <= b
    else
        if type(a) ~= "table" or getmetatable(a) ~= UTFString_mt then error("attempt to compare " ..
...
-- tables for UTF-8 case conversion
